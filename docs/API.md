...
### `GET /thumb/{photo_id}`
Returns the 400×400 JPEG thumbnail bytes for `photo_id` (16 hex chars), cached at `thumbs/{photo_id}.jpg` in the storage backend. EXIF orientation is baked in at generation time (`ImageOps.exif_transpose`).

**In production this endpoint is served by the Cloudflare Worker isolate, not the container** (see issue #51 / `worker/thumb.ts`):

- The Worker verifies auth (JWT + email allowlist, ported from `app/api/auth.py`) on **every** request, then checks the edge cache (`caches.default`), then reads the object from the `R2_THUMBS` binding.
- A hit returns the JPEG bytes directly with `Content-Type: image/jpeg` and `Cache-Control: public, max-age=31536000, immutable` (safe because `photo_id` is a content hash, so a key never maps to different bytes). Responses are stored at the edge via `ctx.waitUntil(caches.default.put(...))`, so repeat/cross-user requests are served from cache.
- If the object isn't in R2 yet (thumb not pre-generated by `--step thumb`), the Worker **falls through to the container** `/thumb/{photo_id}` endpoint below, which generates it on demand. The 302 from that path is not cached.

The container endpoint (`app/api/main.py`) is now only that rare fall-through generator. It returns a **302 redirect** to a short-TTL presigned URL: with `STORAGE_BACKEND=r2` the redirect points to R2 directly; with `STORAGE_BACKEND=local` it points to `/local/<key>` served as static files. (When auth is disabled or running purely against the container, this is also the path you get.)
...
| 400 | Invalid `date_from`/`date_to` format, invalid `people_mode`, or malformed `/thumb/{id}` (not 16 hex chars) |
